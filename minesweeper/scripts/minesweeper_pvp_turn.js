...
*/
    ...
    if(game_status == 0 && clickedSmileyFlag == 0)
    {
      ...
      //alert("detected! game_status = " + game_status + "and clickedSmileyFlag == " + clickedSmileyFlag);
  ...
  function endGame(outcome) {
      ...
      if(player == 2 || player == 4 || player == 0 || player == -1)
      {
        wait();
      }
  ...
  function updateCSS()
  {
      if(isLoaded == 1)
      {
        ...
        else if(player == 3 || player == 4)
        {
          ...
          if(player_turn > 4 && player == 3)
          {
            $.ajax({url: "../php/insertGames.php?func=changeTurn&playerid=3&gameid=" + game_id});
          }
      }
    }
...
*/
      //check to see who the tile belongs to that was clicked
      $.ajax({url: "../php/getPlayers.php?func=playerTile&gameid=" + game_id + "&tileid=" + thisSquare, 
        success: function(result) {
        player_tile = result;
      },async: false});
...
*/
      ...
      if(player == player_tile)
      {
       ...
       //and if we pass maxplayers, reset it back to player 1 or 3 turn.
       if(player == 2)
       {
         $.ajax({url: "../php/getPlayers.php?func=playerTurn&gameid=" + game_id,
           success: function(result) {
  ...
  function fetchPlayerStats(p)
  {
    ...
    $.ajax({url: "../php/getPlayers.php?func=gsrTime&player=" + p,
                success: function(result) {
                        window.GSRManager.update(p, result);
                }  } );
    $.ajax({url: "../php/getPlayers.php?func=hrTime&player=" + p,
                success: function(result) {
                        window.HeartRateManager.update(p, result);
                }  } );
    $.ajax({url: "../php/getPlayers.php?func=skinTime&player=" + p,
                success: function(result) {
                        window.SkinTempManager.update(p, result);
                }  } );
    $.ajax({url: "../php/getPlayers.php?func=getgame&player=" + p,
                dataType: "json",
                success: function(result) {
  ...
  function layMines() 
  {
      ...
      else if(player == 3 || player == 4 || player == -1)
      {
        $.ajax({url: "../php/getPlayers.php?func=getGameIdQuad&player1id=3&player2id=4",
          success: function(result) {
          game_id = result;
...
/*
  function isLoadedTrue()
  {
        setTimeout("isLoadedTrue()", 3000); // allow game to fully load
  }
*/
  function erase() {
  ...
  {
    if((player==1 || player==3) && clickedSmileyFlag == 0)
    {
      clickedSmileyFlag = 1;
      document.getElementById("loadingId").innerHTML = "Game Over...";
...
/*
        ...
        {
          $.ajax({url: "../php/insertGames.php?func=turn&gameid=" + (game_id+1) + "&total=" + total + "&gametype=3"});
        }
...
*/
  ...
  function waitForTiles()
  {
    ...
    //ADDING TILES TO DATABASE : QUAD BASED
    //gets the current game id by checking the latest row in descending order
    ...
    if(game_status == 0 || game_status == -1)
    {
      ...
      if(player == 1)
      {
        $.ajax({url: "../php/insertGames.php?func=turn&gameid=" + (game_id+1) + "&total=" + total + "&playerid=1" + "&gametype=3"});
        $.ajax({url: "../php/getPlayers.php?func=getGameIdQuad&player1id=1&player2id=2",
          success: function(result) {
          game_id = result;
        },async: false});
      ...
      {
        $.ajax({url: "../php/insertGames.php?func=turn&gameid=" + (game_id+1) + "&total=" + total + "&playerid=3" + "&gametype=3"});
        $.ajax({url: "../php/getPlayers.php?func=getGameIdQuad&player1id=3&player2id=4",
          success: function(result) {
          game_id = result;
        },async: false});
      }
      //resetting game to player 1 or player 3 turn
  ...
  { 
    if(isLoaded == 1)
    {
      ...
      update_square = [];
      //console.log("game updated | time = .5 seconds");
    }
...
*/
      if(player == 1 || player == 2 || player == 0)
      {
        //checks if explosion has occured
...
*/
      ...
      if(player == 3 || player == 4 || player == -1)
      {
        ...
        //restart the game if host is entering new
/*
        if(player == 3 && game_status != 0 && game_status != -1)
        {
          $.ajax({url: "../php/getPlayers.php?func=getGameIdQuad&player1id=3&player2id=4", 
            success: function(result) {
            game_id = result;
